...
# Chuẩn Pythonic trong thực tế: Thư viện chuẩn `bisect` (Binary Search ở tầng C)
import bisect

# bisect_left tìm vị trí đầu tiên chèn target mà vẫn giữ nguyên thứ tự sắp xếp:
idx = bisect.bisect_left(sorted_list, 9)
found = idx < len(sorted_list) and sorted_list[idx] == 9
print(f"bisect find 9 (found={found}):", idx if found else -1)  # 2
