...
# Duck Typing an toàn: Các class không cần kế thừa trực tiếp từ Protocol (No explicit inheritance).
#    Chỉ cần có đầy đủ các phương thức tương ứng là tự động tương thích kiểu (Structural Subtyping).
# Giảm tính phụ thuộc chặt chẽ (Decoupling): Dễ dàng viết Mock / Plugin mà không cần sửa class gốc.
...
# Triển khai KuzuDB Storage (Không cần kế thừa GraphStorage)
class KuzuStorage:
    def save_node(self, node_id: str, properties: dict) -> bool:
