# 1. O(1) - Constant Time: Thời gian thực thi không đổi dù n tăng
def check_even_or_odd(number):
    return "Even" if number % 2 == 0 else "Odd"

print("O(1) Check:", check_even_or_odd(42))

# 2. O(n) - Linear Time: Thời gian tăng tuyến tính tỉ lệ thuận với kích thước n
...
# 3. O(n^2) - Quadratic Time: 2 vòng lặp lồng nhau (Nested loops)
def count_pairs(n):
    count = 0
    for i in range(n):
        for j in range(n):
...
# 4. Tóm tắt các cấp độ Big O (từ nhanh nhất đến chậm nhất):
